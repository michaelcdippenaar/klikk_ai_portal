...
def create_dashboard_widget(
    ...
    **extra_kwargs,
    ...
    from widget_store import _extract_data_source
    data_source = _extract_data_source(widget_type, props)
    ...
    # Save widget to DB so the refresh engine can pick it up
    try:
        import widget_store
        # Find default page
        ...
        if target_page_id:
            widget_store.create_widget(target_page_id, widget_config)
            if data:
                widget_store.save_widget_data(widget_id, data)
    except Exception:
        pass  # Don't fail widget creation if DB save fails
