...
def tm1_get_element_attributes(
    ...
    for a in attr_defs:
        try:
            bulk = tm1.elements.get_attribute_of_elements(dimension_name, hier, a.name)
...
def tm1_get_element_attributes_bulk(
    ...
    # Bulk-fetch each attribute (one API call per attribute, not per element)
    attr_data: dict[str, dict] = {}
    for attr in attributes:
        try:
            attr_data[attr] = tm1.elements.get_attribute_of_elements(dimension_name, hier, attr)
        except Exception:
            attr_data[attr] = {}
...
def tm1_export_dimension_attributes(
    ...
    # Bulk-fetch each attribute for all elements (one API call per attribute)
    attr_data: dict[str, dict[str, Any]] = {}
    for attr in attr_names:
        try:
            attr_data[attr] = tm1.elements.get_attribute_of_elements(
...
def tm1_find_element(
    ...
    matches = []
    seen = set()  # (dim, element) to avoid duplicates
        ...
        try:
            ...
            for el in elements:
                if search_lower in el.lower():
                    ...
                    if key not in seen:
                        seen.add(key)
                        matches.append({"dimension": dim, "element": el, "matched_on": "element_name"})
                        if len(matches) >= 50:
                            return {"search_term": search_term, "matches": matches, "truncated": True}

            # Search alias/attribute values
            if search_aliases:
                try:
                ...
                for attr_def in attr_defs:
                    ...
                    try:
                        attr_values = tm1.elements.get_attribute_of_elements(dim, dim, attr_name)
                    except Exception:
                        continue
    ...
    return {"search_term": search_term, "matches": matches, "count": len(matches)}
