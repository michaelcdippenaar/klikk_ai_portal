...
def _extract_dimension_filters(
    ...
    if "entity" in cube_dimensions:
        try:
            for attr in ALIAS_ATTRS.get("entity", ["name"]):
                try:
                    aliases = tm1.elements.get_attribute_of_elements("entity", "entity", attr)
                except Exception:
                    continue
                query_lower = query.lower()
                for el_name, alias in aliases.items():
                    ...
                    # Exact alias in query, or query word matches alias
                    ...
                    if alias_lower in query_lower:
                        ...
                        # Check if any alias word (3+ chars) appears in query
    ...
    if "listed_share" in cube_dimensions and "listed_share" not in filters:
        try:
            for attr in ALIAS_ATTRS.get("listed_share", ["share_name"]):
                try:
                    aliases = tm1.elements.get_attribute_of_elements("listed_share", "listed_share", attr)
                except Exception:
                    continue
                query_lower = query.lower()
                for el_name, alias in aliases.items():
