...
def ai_setup_refresh_rag(
    ...
    if not settings.voyage_api_key:
        return {"error": "VOYAGE_API_KEY not configured. Cannot embed chunks."}
    ...
    try:
        ...
        # Chunk collection
        voyage_client = ix["voyageai"].Client(api_key=settings.voyage_api_key)
        conn = ix["get_pg_conn"]()
        chunks = []
        ...
        # Embed and upsert
        log.info("ai_setup: embedding %d chunks via VoyageAI", len(chunks))
        embedded = ix["embed_chunks"](chunks, voyage_client)
...
def ai_setup_seed_facts() -> dict[str, Any]:
    ...
    try:
        ...
        return {"error": "context_store module not available"}
