...
def gdrive_index_folder(folder_id: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        if not settings.voyage_api_key:
            return {"error": "VOYAGE_API_KEY not set — cannot embed documents."}
        ...
        for file_info in files:
            try:
                ...
                # Chunk the text
                from rag.chunker import chunk_markdown, chunk_plain_text
                ...
                for i in range(0, len(chunks), 128):
                    ...
                    resp = voyage_client.embed(texts, model=settings.voyage_model, input_type="document")
